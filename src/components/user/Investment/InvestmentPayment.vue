<template>
  ...
  <NotificationPopup
      ...
      :amount="testAmount"
      :transaction-code="testTransactionCode"
...
async function loadPaymentInfo() {
  try {
    const res = await api.get(`/investment/branches/payment?packageId=${packageId}`);
    const data = res.data;
...
onMounted(async () => {
  ...
  socketService.subscribe(
      ...
      async (event) => {
        if (!event || event.domain !== "INV_CHECKOUT") return;

        if (event.type === "PAYMENT_SUCCESS") {
          status.value = "success";

          // Hiển thị notification với dữ liệu thật từ server
          showNotification.value = true;
          testAmount.value = event.amount || amount.value;
          testTransactionCode.value = event.transactionCode || transactionCode.value;
          testTimestamp.value = event.timestamp || new Date().toISOString();

          // Optional: tự động chuyển trang sau 5 giây
          // setTimeout(() => {
...
// Thêm các biến cho test notification
const showNotification = ref(false)
const testAmount = ref(50000000) // 50 triệu VNĐ
const testTransactionCode = ref('INV-' + Date.now())
const testTimestamp = ref(new Date().toISOString())


// Function to show test notification
const showTestNotification = () => {
  // Tạo dữ liệu test
  testAmount.value = Math.floor(Math.random() * 100000000) + 10000000 // Random 10-100 triệu
  testTransactionCode.value = 'INV-' + Date.now() + '-' + Math.random().toString(36).substr(2, 6).toUpperCase()
  testTimestamp.value = new Date().toISOString()
  ...
  console.log('Test notification shown:', {
    ...
    code: testTransactionCode.value,
