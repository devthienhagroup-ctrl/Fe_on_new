<template>
  <div class="team-chat card border-0 shadow-sm">
    <div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center">
      <div>
        <h5 class="fw-semibold mb-0 text-dark">Thảo luận</h5>
      </div>
    ...
    <div class="card-body p-0 d-flex flex-column">
      <div class="chat-messages" ref="messageContainer">
        <div v-if="loading" class="text-center text-muted py-5">
          <i class="fa-solid fa-spinner fa-spin me-1"></i> Đang tải lịch sử trò chuyện...
        </div>
        <div v-else>
          <!--          <button-->
          <!--              v-if="!loading && messages?.length && hasMore"-->
          ...
          message.isRecalled ? 'recalled' : '',
            ...
            >
              <div class="message-header mb-1">
                <strong>{{ message.author?.fullName || message.author?.name }}</strong>
              </div>

              <!-- Ẩn hoàn toàn nội dung khi thu hồi -->
              <template v-if="!message.isRecalled">
                <div v-if="message.replyTo" class="reply-preview" @click="jumpToMessage(message.replyTo.id)">
                  <div class="small text-muted">Trả lời
                    {{ message.replyTo.author?.fullName || message.replyTo.author?.name }}
                  </div>
                  <div class="fw-semibold text-truncate">{{ message.replyTo.content }}</div>
                </div>
                ...
                <div v-if="message.attachments?.length" class="attachments">
                  <template v-for="(file, index) in message.attachments" :key="index">
                    <a
                      ...
                      <img :src="getAttachmentUrl(file) || '/placeholder-image.jpg'" :alt="file.name || 'attachment'"/>
                    </a>
                    <a
                        v-else
                        :href="getAttachmentUrl(file) || '#'"
                        ...
                        :class="{ 'disabled-attachment': !getAttachmentUrl(file) }"
                        :ref="(el) => observeAttachment(file, el)">
                      <i class="fa-solid fa-paperclip me-2"></i>{{ file.name || 'Tập tin đính kèm' }}
                    </a>
                  </template>
                </div>
              ...
              <!-- Hiển thị khi tin nhắn đã thu hồi -->
              <template v-else>
                <div class="message-content recalled-content">
                  <i class="fa-solid fa-ban me-2 text-muted"></i>
                  <span class="text-muted fst-italic">Tin nhắn đã được thu hồi</span>
                </div>
                <div class="message-footer d-flex justify-content-between align-items-center mt-1">
                  <small class="text-muted">{{ formatDate(message.createdAt) }}</small>
                </div>
      ...
      <div class="composer border-top">
        <div v-if="replyTarget" class="replying-to">
          <div>
            Đang trả lời <strong>{{ replyTarget.author?.fullName || replyTarget.author?.name }}</strong>
            <p class="mb-0 small text-muted text-truncate">{{ replyTarget.content }}</p>
          </div>
        ...
        <div class="mention-dropdown-container" v-if="showMentionDropdown">
          <div class="mention-dropdown">
            ...
            <div class="mention-list">
              ...
              >
                <img v-if="member.avatar" :src="member.avatar" class="mention-avatar" alt="avatar"/>
                <div v-else class="mention-avatar no-avatar">
                  <i class="fa-solid fa-user"></i>
                </div>
                <div class="d-flex flex-column align-items-start flex-grow-1">
                  <span>{{ member.fullName || member.name }}</span>
                  <small class="text-muted">@{{ normalizeName(member.fullName || member.name) }}</small>
                </div>
              </button>
        ...
        <div class="composer-input">
          ...
          <div class="composer-toolbar">
            ...
            <button class="btn btn-thg" :disabled="sending || !canSend" @click="handleSend">
...
import { isImageFile } from '../../../../utils/fileUrl.js'
import { loadPrivateFileDataUrl, resolveFileId, getCachedFileDataUrl } from '../../../../api/fileApi.js'
...
const filteredMembers = computed(() => {
  ...
  if (mentionSearchText.value) {
    ...
    members = members.filter(member => {
      ...
      const normalized = normalizeName(member.fullName || member.name).toLowerCase()
      return name.includes(searchTerm) || normalized.includes(searchTerm)
    })
  }
...
// Hàm chuẩn hóa tên: bỏ dấu tiếng Việt và khoảng trắng
...
const isMine = (message) => {
  ...
  const candidate = message.authorId ?? message.author?.id ?? message.author?.idE
...
const handleKeydown = (event) => {
  if (showMentionDropdown.value) {
    ...
    // Trigger mention dropdown khi gõ @
    setTimeout(() => {
...
const selectMention = (index) => {
  ...
  if (index === -1) {
    ...
    mentionText = normalizeName(member.fullName || member.name)
  }
...
// Mention từ nút @ cũ - SỬA LẠI: không thêm @ vì đã có trong mentionText
const insertMentionFromDropdown = (member) => {
  // Sử dụng tên đã được chuẩn hóa, KHÔNG thêm @ vì đã có trong text
  const normalizedName = normalizeName(member.fullName || member.name)
  newMessage.value = `${newMessage.value}@${normalizedName} `
