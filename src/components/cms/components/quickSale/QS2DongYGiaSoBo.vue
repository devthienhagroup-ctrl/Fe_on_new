<template>
  <div class="pricing-editor">
    ...
    <div class="editor-header">
      <h2>Quản lý Đồng ý giá sơ bộ</h2>
    ...
    <div class="navigation-tabs">
      ...
      >
        <i class="fas fa-border-style"></i> Layout & Border
      </button>
    </div>
    ...
    <div class="tab-content">
        ...
        <div class="settings-grid">
          ...
          <div class="setting-section">
            ...
            <div class="form-group">
              <label for="section-id">ID Section</label>
              <input
                  type="text"
                  id="section-id"
                  v-model="content.id"
                  placeholder="agree-price"
              />
            </div>

            <div class="form-group">
              <label for="order-number">Thứ tự hiển thị</label>
              <input
                  type="number"
                  id="order-number"
            ...
            <div class="form-group">
              <label for="border-radius">Bo góc section</label>
              <div class="input-with-unit">
                <input type="text" id="border-radius" v-model="content.borderRadius" placeholder="20px" />
                <span class="unit">px</span>
              </div>
            </div>

            <div class="form-group">
              <label for="box-shadow">Đổ bóng</label>
              <input
                  type="text"
                  id="box-shadow"
                  v-model="content.boxShadow"
                  placeholder="0 0 10px rgba(0, 0, 0, 0.1)"
              />
            </div>

            <div class="form-group">
              <label for="padding">Padding section</label>
              <input
                  type="text"
                  id="padding"
                  v-model="content.padding"
                  placeholder="20px 20px 20px 20%"
              />
            </div>

            <div class="form-group">
              <label for="title-position">Vị trí tiêu đề</label>
              <select id="title-position" v-model="content.titlePosition">
          ...
          <div class="setting-section">
            ...
            <div class="form-group">
              <label for="mobile-padding">Padding mobile</label>
              <input
                  type="text"
                  id="mobile-padding"
                  v-model="content.layout.mobileSettings.padding"
                  placeholder="20px 20px 150px"
              />
            </div>

            <div class="form-group">
              <label class="checkbox-label">
                <input type="checkbox" v-model="content.layout.mobileSettings.img1rbDisplay" true-value="none" false-value="block" />
            ...
            <div class="form-group">
              <label for="mobile-flex-direction">Hướng layout mobile</label>
              <select id="mobile-flex-direction" v-model="content.layout.mobileSettings.flexDirection">
                <option value="column">Dọc</option>
                <option value="row">Ngang</option>
                <option value="row-reverse">Ngang đảo ngược</option>
                <option value="column-reverse">Dọc đảo ngược</option>
              </select>
            </div>
          </div>
        </div>
          ...
          <div class="setting-section">
            ...
            <div class="form-group" v-if="!iconPreview && !content.icon">
              ...
              <input
                  ...
                  placeholder="/imgs/icon-dong-y-gia-so-bo.png"
              />
            </div>
          ...
          <div class="setting-section">
            ...
            <div class="form-group">
              <div class="label" style="display: flex; justify-content: space-between">
                ...
                <AdvancedEditModal
                    v-model="content.textContent.paragraph"
                ></AdvancedEditModal>
              </div>
              ...
              ">
                <div class="tiptap" v-html="content.textContent.paragraph"></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'images'" class="images-settings">
        <div class="settings-grid">
          <!-- Hình ảnh 1 -->
          <div class="setting-section">
            <h3><i class="fas fa-image"></i> Hình ảnh chính (img1)</h3>
            ...
            <div class="form-group" v-if="!image1Preview && !content.images.img1.src">
              <label for="image1-url">Hoặc nhập URL ảnh</label>
              <input
                  type="text"
                  id="image1-url"
                  v-model="content.images.img1.src"
                  placeholder="/imgs/anh-dong-y-1.png"
              />
            </div>

            <div class="form-group">
              <label for="image1-alt">Alt text cho ảnh</label>
              <input
                  type="text"
                  id="image1-alt"
                  v-model="content.images.img1.alt"
                  placeholder="Đồng ý giá sơ bộ 1"
              />
            </div>

            <div class="form-group">
              <label for="image1-class">CSS Class</label>
              <input
                  type="text"
                  id="image1-class"
                  v-model="content.images.img1.className"
                  placeholder="img1 fade-left"
              />
            </div>

            <div class="form-group">
              <label for="image1-zindex">Z-Index</label>
              <input
                  type="number"
                  id="image1-zindex"
                  v-model.number="content.images.img1.zIndex"
                  min="0"
                  max="10"
              />
            </div>
          ...
          <!-- Hình ảnh phụ (img1rb) -->
          <div class="setting-section">
            <h3><i class="fas fa-layer-group"></i> Hình ảnh phụ (img1rb)</h3>

            <div class="file-upload compact">
              <div class="file-upload-area" @click="triggerImage1rbInput" @dragover.prevent @drop.prevent="handleImage1rbDrop">
                <input
                    type="file"
                    ref="image1rbInput"
                    @change="handleImage1rbUpload"
                    accept="image/*"
                    style="display: none"
                />
                <i class="fas fa-cloud-upload-alt"></i>
                <p>Kéo thả ảnh phụ vào đây</p>
              </div>

              <div v-if="image1rbPreview || content.images.img1rb.src" class="upload-preview">
                <img :src="getImageUrl(content.images.img1rb.src)" alt="Preview" />
                <div class="preview-actions">
                  <button class="btn-preview-action" @click="previewImage1rb" title="Xem trước">
                    <i class="fas fa-eye"></i>
                  </button>
                  <button class="btn-preview-action" @click="removeImage1rb" title="Xóa ảnh">
                    <i class="fas fa-trash-alt"></i>
                  </button>
                </div>
              </div>
            ...
            <div class="form-group" v-if="!image1rbPreview && !content.images.img1rb.src">
              <label for="image1rb-url">URL ảnh phụ</label>
              <input
                  type="text"
                  id="image1rb-url"
                  v-model="content.images.img1rb.src"
                  placeholder="/imgs/anh-dong-y-1-rb.png"
              />
            </div>

            <div class="form-group">
              <label for="image1rb-alt">Alt text cho ảnh phụ</label>
              <input
                  type="text"
                  id="image1rb-alt"
                  v-model="content.images.img1rb.alt"
                  placeholder="Đồng ý giá sơ bộ 1 rb"
              />
            </div>

            <div class="form-group">
              <label for="image1rb-class">CSS Class</label>
              <input
                  type="text"
                  id="image1rb-class"
                  v-model="content.images.img1rb.className"
                  placeholder="img1rb fade-left"
              />
            </div>

            <div class="form-group">
              <label for="image1rb-zindex">Z-Index</label>
              <input
                  type="number"
                  id="image1rb-zindex"
                  v-model.number="content.images.img1rb.zIndex"
                  min="0"
                  max="10"
              />
            </div>
            ...
            <div class="form-group">
              <label class="checkbox-label">
                <input type="checkbox" v-model="content.images.img1rb.showOnMobile" />
                <span class="checkmark"></span>
          ...
          <!-- Hình ảnh 2 -->
          <div class="setting-section">
            <h3><i class="fas fa-image"></i> Hình ảnh 2 (img2)</h3>
            ...
            <div class="form-group" v-if="!image2Preview && !content.images.img2.src">
              <label for="image2-url">URL ảnh 2</label>
              <input
                  type="text"
                  id="image2-url"
                  v-model="content.images.img2.src"
                  placeholder="/imgs/anh-dong-y-2.png"
              />
            </div>

            <div class="form-group">
              <label for="image2-alt">Alt text cho ảnh 2</label>
              <input
                  type="text"
                  id="image2-alt"
                  v-model="content.images.img2.alt"
                  placeholder="Đồng ý giá sơ bộ 2"
              />
            </div>

            <div class="form-group">
              <label for="image2-class">CSS Class</label>
              <input
                  type="text"
                  id="image2-class"
                  v-model="content.images.img2.className"
                  placeholder="img2"
              />
            </div>

            <div class="form-group">
              <label>Vị trí ảnh 2</label>
              <div class="position-controls">
                <div class="position-row">
                  <div class="position-input">
                    <label>Bottom</label>
                    <input type="text" v-model="content.images.img2.position.bottom" placeholder="-200px" />
                  </div>
                  <div class="position-input">
                    <label>Right</label>
                    <input type="text" v-model="content.images.img2.position.right" placeholder="-100px" />
                  </div>
                </div>
              </div>
            </div>
      ...
      <div v-if="activeTab === 'layout'" class="layout-settings">
        <div class="settings-grid">
          <!-- Border Settings -->
          <div class="setting-section">
            <h3><i class="fas fa-border-style"></i> Cài đặt Border</h3>

            <div class="color-input-group">
              <label>Màu border</label>
              <div class="color-input form-group">
                <input type="color" v-model="content.layout.borderColor" />
                <input type="text" v-model="content.layout.borderColor" placeholder="#C2CBF0" />
              </div>
            </div>
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <div class="input-with-unit">
                  <input type="text" id="border-width" v-model="content.layout.borderWidth" placeholder="5px" />
                  <span class="unit">px</span>
                </div>
              </div>
              <div class="form-group">
                <label for="border-style">Kiểu border</label>
                <select id="border-style" v-model="content.layout.borderStyle">
...
const image1rbPreview = ref(null)
...
const loadData = async () => {
  try {
    ...
    if (response.data && response.data.contentJson) {
      ...
      // Assuming data structure has section2
      const sectionData = data.section2 || data
...
const saveChanges = async () => {
  try {
    ...
    const sectionData = {
      id: SECTION_ID,
      name: 'Đồng ý giá sơ bộ',
      contentJson: JSON.stringify(contentData, null, 2),
      sortOrder: content.order || 2,
      files: filesToSend
    }
...
const updateContentUrls = (file) => {
  ...
  if (content.images.img1rb.src === tempUrl) {
    content.images.img1rb.src = realUrl
  }
...
// Image 1rb
const triggerImage1rbInput = () => {
  image1rbInput.value?.click()
}

const handleImage1rbUpload = (event) => {
  const file = event.target.files[0]
  if (file) {
    handleFileUpload(file, 'Hình ảnh phụ 1rb', image1rbPreview, 'images.img1rb.src')
  }
  event.target.value = ''
}

const handleImage1rbDrop = (event) => {
  event.preventDefault()
  const file = event.dataTransfer.files[0]
  if (file) {
    handleFileUpload(file, 'Hình ảnh phụ 1rb', image1rbPreview, 'images.img1rb.src')
...
const removeImage1rb = () => {
  if (content.images.img1rb.src) {
    handleRemoveFile(content.images.img1rb.src, 'images.img1rb.src', image1rbPreview)
  }
}
...
const previewImage1rb = () => {
  if (content.images.img1rb.src) {
    previewImageUrl.value = getImageUrl(content.images.img1rb.src)
    showPreviewModal.value = true
  } else {
    showToast('Không có ảnh phụ để xem trước', 'warning')
  }
}
...
const previewImage2 = () => {
  if (content.images.img2.src) {
    ...
    showToast('Không có ảnh 2 để xem trước', 'warning')
  }
}
